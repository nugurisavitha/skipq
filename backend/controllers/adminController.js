...
const getAnalytics = asyncHandler(async (req, res) => {
  ...
  // Summary: total revenue, avg per day, growth
  const revenueAgg = await Order.aggregate([
    { $match: { ...filter, paymentStatus: 'paid' } },
    { $group: { _id: null, total: { $sum: '$total' }, count: { $sum: 1 } } },
  ]);
  const totalRevenue = revenueAgg[0]?.total || 0;
  const averagePerDay = Math.round(totalRevenue / parseInt(days));

  // Growth: compare current period to previous period
  const prevStart = new Date(startDate);
  prevStart.setDate(prevStart.getDate() - parseInt(days));
  const prevRevenueAgg = await Order.aggregate([
    { $match: { paymentStatus: 'paid', createdAt: { $gte: prevStart, $lt: startDate } } },
    { $group: { _id: null, total: { $sum: '$total' } } },
  ]);
  ...
  const ordersByType = ordersByTypeAgg.map((t) => ({
    type: t._id,
    count: t.count,
  }));

  // Top cuisines (from restaurant cuisine field)
  const topCuisines = await Order.aggregate([
    { $match: filter },
    { $lookup: { from: 'restaurants', localField: 'restaurant', foreignField: '_id', as: 'rest' } },
    { $unwind: '$rest' },
    { $group: { _id: '$rest.cuisine', count: { $sum: 1 } } },
    { $sort: { count: -1 } },
    { $limit: 6 },
  ...
  res.status(200).json({
    success: true,
    data: {
      summary: { totalRevenue, averagePerDay, growthPercent },
      revenueChart,
      ordersByStatus,
      ordersByType,
      topCuisines,
